...
<body>
<?php
if( $this->session->flashdata('regNo') ){
    $flashRegno = $this->session->flashdata('regNo');
    $code = auCommon_fetchPrintingCode( $this->session->flashdata('printNo') );
    ?>
    <script type="text/javascript">
        // $('#insertRecordForm')[0].reset();
        /*https://stackoverflow.com/questions/2914/how-can-i-detect-if-a-browser-is-blocking-a-popup*/
        var newWin = window.open("<?php echo base_url().'index.php/dashboard/patientsRecord/Printing/index/'.$code.'/'.$flashRegno;?>");
    ...
    <?php
    // redirect('dashboard/patientsRecord/Printing/index/'.$code.'/'.$flashRegno,'refresh');
}
?>
	                ...
	                <div class="row" style="align-items: center;margin:10px 0;">
	                    <div class="col-5">
	                        <div class="row">
	                            ...
	                            <div class="col-8" style="padding-left: 10px;padding-right: 18px;">
	        	...
	        	<div class="modal-body" id="searchedModalBody">
                    <div class="table-responsive">
    	          		<table class="table table-striped">
    	          			<thead>
    	          				<tr>
    	          					<td>Reg.no<input type="text" class="patientFilter" data-eq="0"></td>
    	          					<td>Patient Name<input type="text" class="patientFilter" data-eq="1"></td>
    	          					<td>Contact no<input type="text" class="patientFilter" data-eq="2"></td>
    	          					<td>Dob<input type="text" class="patientFilter" data-eq="3"></td>
    	          					<td>House<input type="text" class="patientFilter" data-eq="4"></td>
                                    <td>Place<input type="text" class="patientFilter" data-eq="5"></td>
                                    <td>Guardian<input type="text" class="patientFilter" data-eq="6"></td>
    	          				</tr>
    	          			</thead>
    	          			<tbody>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$('#searchBox').focus();
		$( "#visitDate" ).datepicker( {format: 'dd-M-yyyy',autoclose:true,startDate:new Date()} );
        $('#update-link').hide();
		...
		$(document).on('dblclick','#searchedModalBody tbody tr',function(){
			...
			var reg = $(this).find('td').eq(0).text();
			if( reg ){
        ...
        $('.patientFilter').on('keyup',function(){
            ...
            var RegNo = $('.patientFilter[data-eq="0"]').val().toLowerCase();
            var PatientName = $('.patientFilter[data-eq="1"]').val().toLowerCase();
            var contactNo = $('.patientFilter[data-eq="2"]').val().toLowerCase();
            var dob = $('.patientFilter[data-eq="3"]').val().toLowerCase();
            var house = $('.patientFilter[data-eq="4"]').val().toLowerCase();
            var place = $('.patientFilter[data-eq="5"]').val().toLowerCase();
            var guardian = $('.patientFilter[data-eq="6"]').val().toLowerCase();


            $("#searchedModalBody tbody tr").hide();

             $('#searchedModalBody tbody tr').each(function(){

                RegNo1 = $(this).find('td').eq(0).text().toLowerCase().indexOf(RegNo);
                PatientName1 = $(this).find('td').eq(1).text().toLowerCase().indexOf(PatientName);
                contactNo1 = $(this).find('td').eq(2).text().toLowerCase().indexOf(contactNo);
                dob1 = $(this).find('td').eq(3).text().toLowerCase().indexOf(dob);
                house1 = $(this).find('td').eq(4).text().toLowerCase().indexOf(house);
                place1 = $(this).find('td').eq(5).text().toLowerCase().indexOf(place);
                guardian1 = $(this).find('td').eq(6).text().toLowerCase().indexOf(guardian);

                if( RegNo1 != -1 && PatientName1 != -1 && contactNo1 != -1 && dob1 != -1 && house1 != -1 && place1 != -1 && guardian1 != -1 ){
                    $(this).show();
		...
		$.ajax({
				...
				success: function(data){
					...
					if(data){
						...
						if(data.length==1 && searchBy == 'reg_card_number'){
	...
	function setSearchedDataInTable( data ){
		...
		for(var i=0;i<trLen;i++){
			html += '<tr>\
						...
						<td>'+data[i].house+'</td>\
                        <td>'+data[i].Place+'</td>\
                        <td>'+data[i].parent_gurdian_name+'</td>\
					</tr>'; 
		}
        ...
        // console.log(search);
        if( searchBy == 'name' ){
            $('#searchedModalBody thead td').eq(1).find('input').val(search);
        }
        else if( searchBy == 'contact_number' ){
            $('#searchedModalBody thead td').eq(2).find('input').val(search);
        }
        else if( searchBy == 'dob' ){
            $('#searchedModalBody thead td').eq(3).find('input').val(search);
        }
        else{

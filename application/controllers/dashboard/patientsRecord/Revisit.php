...
class Revisit extends CI_Controller {
	...
	public function index()
	{
        ...
        if ($this->form_validation->run('insertPatientsRevisitRecord') == FALSE){
            ...
            $reVisitId = auCommon_intSanitize($this->input->post('reVisitId'));
            ...
            if( count( $patientInformation )>0 && isset( $patientInformation[0]->id ) ){ 
                ...
                if( $reVisitId!==0 || $reVisitId!='' )
                {
                    if( $this->precord->updatePatientRecord( $reVisitDetails, 'visit_id', 'visit_details', $reVisitId ) ){
                        $this->session->set_flashdata('message', '<strong>Error!</strong> Record Updated');
                    }
                    else{
                        $this->session->set_flashdata('message', '<strong>Error!</strong> Record not Updated');
                    }
                }
                else{
    ...
    public function patientSearchAjax( $searchBy='name', $searchKey='jac')/*searchBy,searchKey=>checking purpose*/
    {
        // echo $searchKey;
        // echo $searchBy;
    ...
    public function patientOldVisitDetailsAjax( $regNo='D00041'){/*regNo=>checking purpose*/
        ...
        if( $patientid!=0 ){
            ...
            foreach ($details as $key => $value) {
                ...
                $status = auCommon_statusTest( $value->status );
                ...
                $result .= '<td>'.$status.'</td>';
                $result .= '<td><button type="button" class="btn au-edit-btn ' . $classDisabled . '">Edit</button></td>';
                $result .= '</tr>';
